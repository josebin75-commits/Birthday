...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (letterId) {
    if (letterId.startsWith('local_')) {
      ...
      try {
        ...
        const letterRef = db.ref("letters/" + letterId);
        ...
        const snapshot = await Promise.race([
          letterRef.once("value"),
          timeoutPromise
        ]);

        if (snapshot.exists()) {
          loadedLetterData = snapshot.val();
          setupLetterUI();
        } else {
    ...
    // ID가 없을 경우, 로컬 템플릿(CONFIG)이 있으면 즉시 로컬 실행 (로컬 더블클릭 테스트 유연성 보장)
    loadLocalFallback();
  }
  ...
  passwordForm.addEventListener('submit', (e) => {
    ...
    const correctPassword = loadedLetterData ? (loadedLetterData.password || loadedLetterData.password) : '';
  ...
  function startBgm() {
    ...
    if (loadedLetterData && loadedLetterData.bgmUrl && loadedLetterData.bgmUrl !== '') {
      ...
      localAudio.play().catch(err => {
        console.log('브라우저 오디오 재생 정책에 따라 차단되었습니다. 사용자 조작 후 재생됩니다.', err);
        isBgmPlaying = false;
        updateBgmUI();
      });
    } else {
  ...
  function initSlideshow() {
    const photos = (loadedLetterData && (loadedLetterData.photoUrls || loadedLetterData.photos)) ? (loadedLetterData.photoUrls || loadedLetterData.photos) : [];
    
    if (photos.length === 0) {
      const fallback = document.createElement('div');
      fallback.className = 'slide-img active';
      fallback.style.background = '#eee';
      fallback.innerHTML = `<div style="padding:20px; text-align:center;">사진이 업로드되지 않았습니다.</div>`;
  ...
  function startTypewriter() {
    const letterText = loadedLetterData ? (loadedLetterData.letterText) : '생일 축하해!';
    let index = 0;
  ...
  replyForm.addEventListener('submit', async (e) => {
    ...
    try {
      await db.ref("replies/" + letterId).push({
        senderName: senderName,
        replyText: replyTextVal,
        createdAt: new Date().getTime()
      });
